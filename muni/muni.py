...
while True:
    ...
    url = f"https://api.511.org/transit/StopMonitoring?api_key={tokens[tokens_index]}&stopCode=15539&agency=SF&Format=json"
    payload={}
    headers = {
    ...
    if "The allowed number of requests" in response.text:
        ...
        if not nested_data: 
            ...
            if arrival_times[0] == 1: 
                add_s = ""
            else:
                add_s = "s" 

            led_txt = f"Next 14 in {arrival_times[0]} minute{add_s}"
